    ...
    try {
      ...
      if (accessToken && refreshToken) {
        const rawResponse = await fetch(
          `http://192.168.1.19:3000/getCurrentlyPlaying?access_token=${accessToken}&refresh_token=${refreshToken}`,
        );
        ...
        if (response.status === 200) {
          ...
          const rawResponse = await fetch(
            `http://192.168.1.19:3000/refresh_token?refresh_token=${refreshToken}`,
          );
          const response = await rawResponse.json();
      ...
      console.error({error: error, status: status, body: body});
    }
  }
